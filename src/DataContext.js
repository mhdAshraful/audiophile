...
const reducer = (state, action) => {
	switch (action.type) {
		...
		case "GRAND_TOTAL":
			...
			let { grand_total, quantity } = state.cart.reduce(
				(accum, item) => {
...
export const DataProvider = ({ children }) => {
	...
	const url = "localhost:5555/api/allproducts";
	 ...
	 *@return type
	 *========================================================================**/
	const getearphone = (data) => {
		dispatch({ type: "GET_EARPHONE", payload: data });
	};
	const getheadphone = (data) => {
		dispatch({ type: "GET_HEADPHONE", payload: data });
	};
	const getspeaker = (data) => {
		dispatch({ type: "GET_SPEAKER", payload: data });
	};
	const increase = (id) => {
	...
	const getData = useCallback(() => {
		console.log("context called from useCallback");
	...
	return (
		<DataContext.Provider
			value={[stored, increase, decrease, addtocart, remove, clearcart]}
		>
			{children}
		</DataContext.Provider>
	);
